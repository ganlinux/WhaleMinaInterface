...
public class EMMSPushManager  {
	 ...
	 * @param account 用户唯一ID
	 */
	public static  void setRefreshRoomsDataRealtime(Context context,String ID,String roomnum){
		SentBody sent = new SentBody();
		sent.setType("realtime_roomview_request");
		sent.setSender(ID);
		sent.setReciever("0");
		sent.put("username",roomnum);
		sendRequest(context,sent);
	 ...
	 * @param account 用户唯一ID
	 */
	public static  void setNlbDataRequest(Context context,String id,String room,String starttime,String endtime){
		SentBody sent = new SentBody();
		sent.setType("nlbdata_request");
		sent.setSender(id);
		sent.setReciever("0");
		sent.put("metertype","1");
		sent.put("room",room);
